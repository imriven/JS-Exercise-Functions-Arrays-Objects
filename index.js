...
return {"id":idparam, "name":nameparam, "email":emailparam}
}
...
return {name:name, sum:(a, b) => a + b, speak:() => `Hello, my name is ${name}`};
}
*/

function makeSmartPerson(name) {
  let personObject = {name:name};
  personObject.sum = function(a, b) {
    return (a + b);
  }
  personObject.speak = function() {
...
function getLastCarInfo(inventory) {
  const thisIndex = inventory.length -1;
  const lastCar = inventory[thisIndex];
...
function getModelYears(inventory) {
  let carEmptyArray = [];
  for (let car of inventory) {
    carEmptyArray.push(car.car_year);
  }
  return carEmptyArray;
}
...
function getOlderCars(inventory, maxYear) {
  ...
  for(let car of inventory) {
    if (car.car_year <= maxYear) {
      resultsCarArray.push(car);
    }
  }
  return resultsCarArray;
}
...
function getGermanCars(inventory) {
  const germanCarMakes = ["Audi", "Mercedes-Benz", "Volkswagen", "BMW"]
  let germanCarArray = [];
  for (let car of inventory) {
    if (germanCarMakes.includes(car.car_make)) {
      germanCarArray.push(car);
    }
  }
  return germanCarArray;
...
*/
function carMaker(odometerParam) {
  let car = {odometer:odometerParam}
  car.drive = distance => {
    car.odometer += distance; //shorthand for car.odometer = car.odometer + distance;
    return car.odometer;
  } 
  return car
}
